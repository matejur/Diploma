...
class Trainer(object):
    ...
    # Spremeni da bo delalo za več batchov
    ...
    def _initDataloader(self):
        ...
        train_pv_loader = pc_processor.dataset.PerspectiveViewLoaderTransformer(
            dataset=trainset,
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, (image, pointcloud, input_label) in enumerate(dataloader):
            ...
            if mode == "Train":
                preds = self.model(image, pointcloud)

                preds_points = preds[label_mask]
                target_classes = input_label[label_mask]

                # compute loss
                total_loss = self.criterion(preds_points, target_classes)
