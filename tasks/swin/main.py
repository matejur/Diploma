...
class Experiment(object):
    def __init__(self, settings: Option):
        ...
        self.epoch_start = 0
        # init model
        self.model = pc_processor.models.FusionCrossNet(backbone=self.settings.img_backbone)

        # init trainer
        self.trainer = trainer.Trainer(
            self.settings, self.model, self.recorder, self.settings.remaining_epochs)
